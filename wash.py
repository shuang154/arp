...
def convert_to_wifi_format(sorted_records, output_file='wifi.txt'):
    ...
    同时生成lib.txt，用户名去掉前置w，密码去掉前后字母保留中间六位数字，加上前缀njfu和后缀!
    """
    try:
        ...
        # 创建或覆盖wifi.txt文件和lib.txt文件
        script_dir = os.path.dirname(os.path.abspath(__file__))
        wifi_path = os.path.join(script_dir, output_file)
        lib_path = os.path.join(script_dir, 'lib.txt')

        with open(wifi_path, 'w', encoding='utf-8') as wifi_file, open(lib_path, 'w', encoding='utf-8') as lib_file:
            for record in sorted_records:
                # 格式：学号  两个空格  密码
                wifi_line = f"{record['user_id']}  {record['password']}"
                wifi_file.write(wifi_line + '\n')
                
                # 处理用户名和密码
                user_id = record['user_id'].lstrip('w')  # 去掉前置w
                password = ''.join(filter(str.isdigit, record['password']))[:6]  # 提取中间六位数字
                if password:  # 确保密码非空
                    lib_password = f"njfu{password}!"
                    lib_line = f"{user_id}  {lib_password}"
                    lib_file.write(lib_line + '\n')
        
        print(f"\n格式转换完成！")
        print(f"已将 {len(sorted_records)} 条记录写入 {output_file} 和 lib.txt")
        print("wifi.txt 格式：学号  两个空格  密码")
        print("lib.txt 格式：学号  两个空格  njfu+六位数字+!")
